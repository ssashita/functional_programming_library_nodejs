...
const ioToTask = x => new Task((reject, resolve) => resolve(x.unsafePerformIO())); // maybeToTask :: Maybe a -> Task () a
...
const convertTaskIOTaskEither2Task = (t) => {
        return t.chain(ioToTask).join().chain(eitherToTask);
};
...
const convertIOTaskEither2Task = (io) => {
        return ioToTask(io).join().chain(eitherToTask);
};
module.exports = {promiseToTask: promiseToTask, maybeToTask: maybeToTask, eitherToTask: eitherToTask,
        ioToTask: ioToTask,
        chain: chain, map: map, compose: compose,
        Maybe: Maybe,
        List:List,
        Either:Either,
        Right:Right,
